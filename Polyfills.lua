...
-------------------------------------------------
-- DEBUG: Periodic debug disabled - use /celldebug command instead
-------------------------------------------------

-------------------------------------------------
-- WotLK 3.3.5a Party Frame Polyfill
-- SecureGroupHeaderTemplate doesn't create buttons automatically in WotLK
-------------------------------------------------
print("[Cell WotLK Polyfill] Party frame polyfill loading...")

local polyfillFrame = CreateFrame("Frame")
polyfillFrame:RegisterEvent("ADDON_LOADED")
polyfillFrame:RegisterEvent("PLAYER_LOGIN")
polyfillFrame:SetScript("OnEvent", function(self, event, addonName)
    -- Only proceed if this is Cell_Wrath being loaded, or PLAYER_LOGIN
    if event == "ADDON_LOADED" and addonName ~= "Cell_Wrath" then return end

    print("[Cell WotLK Polyfill] "..event.." event fired, creating party buttons...")

    if not Cell or not Cell.frames then
        print("[Cell WotLK Polyfill] ERROR: Cell or Cell.frames not available")
        return
    end

    local header = _G["CellPartyFrameHeader"]
    if not header then
        print("[Cell WotLK Polyfill] ERROR: CellPartyFrameHeader not found")
        return
    end

    -- Check if buttons already exist (retail behavior)
    if #header > 0 then
        print("[Cell WotLK Polyfill] Buttons already exist (NumButtons: "..#header.."), skipping creation")
        return
    end

    print("[Cell WotLK Polyfill] Manually creating party buttons...")
    ...
    for i = 1, 5 do
        ...
        -- Register with header (so ipairs works)
        header[i] = playerButton

        -- Manually assign units since SecureGroupHeaderTemplate doesn't do it in WotLK
        local unit
        if i == 1 then
            unit = "player"
        else
        ...
        print("[Cell WotLK Polyfill] Created button " .. i .. ": " .. buttonName .. " (unit: " .. unit .. ")")
    end
    ...
    C_Timer.After(0.1, function()
        ...
        for i = 1, 5 do
            ...
            if button then
                ...
                if unit then
                    print("[Cell WotLK Polyfill] Registering button " .. i .. " with unit: " .. unit)
    ...
    for i, playerButton in ipairs(header) do
        ...
        if Cell.unitButtons and Cell.unitButtons.party then
    ...
    print("[Cell WotLK Polyfill] Party buttons created successfully")

    -- Trigger a layout update to properly size and position the buttons
    -- Use a short delay to ensure CellDB is fully initialized
    C_Timer.After(0.5, function()
        print("[Cell WotLK Polyfill] Layout update timer fired (0.5s delay)")
        if not Cell or not Cell.Fire or not CellDB then
            print("[Cell WotLK Polyfill] ERROR: Cannot trigger layout update - Cell.Fire or CellDB not available")
            return
        end

        local layoutName = CellDB["general"] and CellDB["general"]["layout"]
        if not layoutName then
            print("[Cell WotLK Polyfill] WARNING: CellDB.general.layout is nil, using 'default'")
            layoutName = "default"
        end

        print("[Cell WotLK Polyfill] Triggering layout update to initialize buttons...")
        print("[Cell WotLK Polyfill] Layout name:", layoutName)

        -- Use nil for 'which' parameter to trigger all layout updates (sizing, positioning, etc.)
        Cell.Fire("UpdateLayout", layoutName, nil)
    end)

    -- Unregister both events since we only need to run this once
    self:UnregisterEvent("ADDON_LOADED")
    self:UnregisterEvent("PLAYER_LOGIN")
end)

-------------------------------------------------
-- WotLK 3.3.5a Party Frame Positioning Polyfill
-- Manually position buttons since SecureGroupHeaderTemplate doesn't do it
-------------------------------------------------
print("[Cell WotLK Polyfill] Positioning polyfill loading...")

local positioningFrame = CreateFrame("Frame")
positioningFrame:RegisterEvent("ADDON_LOADED")
positioningFrame:SetScript("OnEvent", function(self, event, addonName)
    if addonName ~= "Cell_Wrath" then return end

    print("[Cell WotLK Polyfill] ADDON_LOADED event fired, registering positioning callback...")

    if not Cell or not Cell.RegisterCallback then
        print("[Cell WotLK Polyfill] ERROR: Cell.RegisterCallback not available")
        return
    end

    print("[Cell WotLK Polyfill] Registering positioning callback...")
    Cell.RegisterCallback("UpdateLayout", "WotLK_PartyFrame_Positioning", function(layout, which)
        -- Only run for party layout updates
        if not Cell.vars or Cell.vars.groupType ~= "party" then return end
        ...
        -- Only run on arrangement updates, roster changes, or initial load (when which is nil)
        if not which or which == "main-arrangement" or which == "pet-arrangement" or which == "roster" then
            print("[Cell WotLK Polyfill] Positioning buttons - which:", which or "nil", "orientation:", layout["main"]["orientation"], "anchor:", layout["main"]["anchor"])
            ...
            for j = 1, 5 do
                if header[j] then
                    ...
                    if j == 1 then
                        -- First button anchors its 'point' corner to header's 'headerPoint'
                        header[j]:SetPoint(point, header, headerPoint, 0, 0)
                        print("[Cell WotLK Polyfill] Button "..j.." positioned: "..point.." -> header "..headerPoint)
                    else
                        -- Subsequent buttons anchor their 'point' corner to previous button's 'playerAnchorPoint'
                        if orientation == "vertical" then
                        ...
                        print("[Cell WotLK Polyfill] Button "..j.." positioned: "..point.." -> Button"..(j-1).." "..playerAnchorPoint.." spacing: "..playerSpacing)
                    end
    ...
    print("[Cell WotLK Polyfill] Party frame positioning polyfill loaded")

    -- Unregister the event since we only need to run this once
    self:UnregisterEvent("ADDON_LOADED")
end)
